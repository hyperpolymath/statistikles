...
The *symbolic library is real and tested*: 41 statistics modules under
`src/stats/`, a single neural/symbolic dispatch chokepoint
(`src/tools/executor.jl`), and 693 `@test` assertions (runtests + e2e +
property + reference-validation (+advanced) + degenerate-input + guardrail +
executor-router) running under Julia 1.10 CI (`e2e.yml`).
...
* [x] 41 symbolic statistics modules
* [x] executor.jl neural/symbolic boundary
* [x] Test suite over the symbolic layer (693 assertions)
...
* [x] Extend ground-truth validation to the advanced modules
      (`test/reference_validation_advanced_test.jl`)
