...
____
41 statistical modules under `src/stats/`, spanning classical (Descriptive,
...
| **Julia 1.10+** | https://julialang.org (statistical computation, 41 modules)
...
| `src/stats/` | 41 statistical domain modules (descriptive, inferential, correlation, non-parametric, etc.)
...
| `test/` | Test suite (693 @test assertions across runtests.jl and the e2e, property, reference-validation (+advanced), degenerate-input, guardrail, and executor-router suites it includes; reference_validation_test.jl checks against ground truth)
