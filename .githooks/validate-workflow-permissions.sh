...
def expand(path):
    """Return a non-directory path as a one-item list, or the sorted `.yml` and
    `.yaml` files directly inside a directory (so this script can be pointed at
    a fixture directory in a test). No matches yield an empty list."""
