...
cookbook:
    ...
    echo "= statistikles Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/statistikles.1 << EOF
    .TH statistikles 1 "$(date +%Y-%m-%d)" "{{version}}" "statistikles Manual"
    .SH NAME
    statistikles \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/statistikles.1"
...
container-build tag="latest":
    @if [ -f Containerfile ]; then \
        podman build -t statistikles:{{tag}} -f Containerfile .; \
    else \
...
# Run container
container-run *args:
    podman run --rm -it statistikles:latest {{args}}

# Push container image
container-push registry="ghcr.io/hyperpolymath" tag="latest":
    podman tag statistikles:{{tag}} {{registry}}/statistikles:{{tag}}
    podman push {{registry}}/statistikles:{{tag}}
...
doctor:
    @echo "Running diagnostics for statistikles..."
...
heal:
    @echo "Attempting auto-repair for statistikles..."
...
help-me:
    @echo "=== statistikles Help ==="
    @echo "Platform: $(uname -s) $(uname -m)"
    @echo "Shell: $SHELL"
    @echo ""
    @echo "To report an issue:"
    @echo "  https://github.com/hyperpolymath/statistikles/issues/new"
    @echo ""
