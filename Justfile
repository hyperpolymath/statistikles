...
build *args:
    @echo "Building statistease (debug)..."
...
build-release *args:
    @echo "Building statistease (release)..."
...
cookbook:
    ...
    echo "= statistease Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/statistease.1 << EOF
    .TH statistease 1 "$(date +%Y-%m-%d)" "{{version}}" "statistease Manual"
    .SH NAME
    statistease \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/statistease.1"
...
container-build tag="latest":
    @if [ -f Containerfile ]; then \
        podman build -t statistease:{{tag}} -f Containerfile .; \
    else \
...
# Run container
container-run *args:
    podman run --rm -it statistease:latest {{args}}

# Push container image
container-push registry="ghcr.io/hyperpolymath" tag="latest":
    podman tag statistease:{{tag}} {{registry}}/statistease:{{tag}}
    podman push {{registry}}/statistease:{{tag}}
...
doctor:
    @echo "Running diagnostics for statistease..."
...
heal:
    @echo "Attempting auto-repair for statistease..."
...
help-me:
    @echo "=== statistease Help ==="
    @echo "Platform: $(uname -s) $(uname -m)"
    @echo "Shell: $SHELL"
    @echo ""
    @echo "To report an issue:"
    @echo "  https://github.com/hyperpolymath/statistease/issues/new"
    @echo ""
